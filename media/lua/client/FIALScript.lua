...
function calcMulExe(character, perkType, perkLvl, PercRegNum)
	...
	if SandboxVars.FitnessIsALifestyle.Hardcore then
		local mulXP = 0.1--math.floor(100*(((100^((PercRegNum/100)+0.002161)-1)*(SandboxVars.FitnessIsALifestyle.XPMultiplier*(perkLvl+1)))/100))/100 + 1
		character:getXp():addXpMultiplier(perkType,mulXP,perkLvl,10)
	else
		local mulXP = 0.1--math.ceil(100*((PercRegNum*(SandboxVars.FitnessIsALifestyle.XPMultiplier*(perkLvl+1)))/100))/100 + 1
		character:getXp():addXpMultiplier(perkType,mulXP,perkLvl,10)
	end
